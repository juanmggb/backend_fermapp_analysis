...
def integrate_odes(params, t_values, x0, s0, p0):
    ...
    solution = odeint(monod_model, initial_conditions,
                      t_values, args=(mu, Y, Yp, Ks))
...
def mse(params, t_values, x_values, s_values, p_values):
    ...
    error = (np.sum((x_estimated - x_values)**2) + np.sum((s_estimated -
             s_values)**2) + np.sum((p_estimated - p_values)**2))/len(s_values)
    return error
...
def estimate_parameters(t_values, x_values, s_values, p_values):
    ...
    model = ga(function=lambda params: mse(params, t_values, x_values, s_values, p_values), dimension=4, variable_type='real', variable_boundaries=varbound,
...
def generate_plot(best_params, t_values, x_values, s_values, p_values):
    ...
    fig.add_trace(go.Scatter(x=t_values, y=x_values, mode='markers',
                             marker=dict(color='red')))
    fig.add_trace(go.Scatter(x=t_values, y=x_estimated, mode='lines',
                             line=dict(color='red')))

    # Add experimental data and optimized model for S
    fig.add_trace(go.Scatter(x=t_values, y=s_values, mode='markers',
                             marker=dict(color='blue')))
    fig.add_trace(go.Scatter(x=t_values, y=s_estimated, mode='lines',
                             line=dict(color='blue')))

    # Add experimental data and optimized model for P
    fig.add_trace(go.Scatter(x=t_values, y=p_values, mode='markers',
                             marker=dict(color='green')))
    fig.add_trace(go.Scatter(x=t_values, y=p_estimated, mode='lines',
